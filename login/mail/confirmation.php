        ...
        <div class="wrapper wrapper--w790">
            ...
            <div class="card card-5">
                ...
                if (isset($_POST['submit']))
                {
                  $user = $_SESSION['user'];
                  $code = $_POST['code'];
                  $query = $conn->query("SELECT * FROM etudiant WHERE (username LIKE '$user' AND code LIKE '$code' AND confirmation = '')");
                  if ($query->fetch() != 0)
                   {
                     $query1 = $conn->query("UPDATE etudiant SET confirmation = 'OK' WHERE username LIKE '$user'");
                     if ($query1)
                     {
