...
{
$query = $conn->query("SELECT * FROM etudiant WHERE exact_level != ''");
}elseif ($a != NULL && $b == NULL && $c == NULL)
{
$query = $conn->query('SELECT * FROM etudiant WHERE exact_level LIKE "A%"');
}elseif ($a == NULL && $b != NULL && $c == NULL) {
$query = $conn->query('SELECT * FROM etudiant WHERE exact_level LIKE "B%"');
}elseif ($a == NULL && $b == NULL && $c != NULL) {
$query = $conn->query('SELECT * FROM etudiant WHERE exact_level LIKE "C%"');
}elseif ($a != NULL && $b != NULL && $c == NULL) {
$query = $conn->query('SELECT * FROM etudiant WHERE exact_level LIKE "A%" OR exact_level LIKE "B%"');
}elseif ($a != NULL && $b == NULL && $c != NULL) {
$query = $conn->query('SELECT * FROM etudiant WHERE exact_level LIKE "A%" OR exact_level LIKE "C%"');
}elseif ($a == NULL && $b != NULL && $c != NULL) {
$query = $conn->query('SELECT * FROM etudiant WHERE exact_level LIKE "B%" OR exact_level LIKE "C%"');
}elseif ($a == NULL && $b == NULL && $c != NULL) {
$query = $conn->query('SELECT * FROM etudiant WHERE exact_level LIKE "C%"');
}
while($row = $query->fetch()){
    echo '<form action="sendMessage.php?id='.$row['id'].'" method="POST">';
    ...
    echo "<td>".$row['exact_level']."</td>";
      ...
      echo"<td><input type='submit' name='submit' value='Message' class='btn-danger btn'></td>";
  echo "</form>";
