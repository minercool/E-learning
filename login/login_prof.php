	...
	<div class="limiter">
		<div class="container-login100">
          ...
          if (isset($_POST['user']) && isset($_POST['password']))
          {
            ...
            $query = $conn->prepare("SELECT * FROM professor WHERE username LIKE '$user' AND password LIKE '$password'");
						$query->execute();
            if ($query->fetch() != 0)
            {
              $_SESSION['loggedin'] = true;
              $query = $conn->query("SELECT * FROM professor WHERE username LIKE '$user' AND password LIKE '$password'");
              $resultat = $query->fetch();
							$_SESSION['role'] = "prof";
							$_SESSION['user'] = $resultat['username'];
							$_SESSION['id'] = $resultat['idp'];
							$id = $_SESSION['id'];
