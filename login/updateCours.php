       ...
       if( in_array($videoFileType,$extensions_arr) && in_array($imageFileType,$extensions_arr) ){
          ...
          if(($_FILES['vid']['size'] >= $maxsize) || ($_FILES["vid"]["size"] == 0)) {
            ...
            if(move_uploaded_file($_FILES['vid']['tmp_name'],$target_file_video) && move_uploaded_file($_FILES['image']['tmp_name'],$target_file_image) && move_uploaded_file($_FILES['image1']['tmp_name'],$target_file_image1) && move_uploaded_file($_FILES['image2']['tmp_name'],$target_file_image2)){
              // Insert record
              $title = $_POST['title'];
              $query = $conn->query("UPDATE cours SET title='$title',image='$target_file_image',image1='$target_file_image1',image2='$target_file_image2',video='$target_file_video',quiz='$location',question_1='$q1',question_2='$q2',cours_level='$cours_level' WHERE idc LIKE '$idc'");
              if($query)
              {
 ...
 <body>
	<main class="page landing-page">
		...
		<div class="row justify-content-md-center">
		  <div class="col-lg-8">
				<h1 class="text-warning text-center" >UPDATE COURSE</h1><br><br>
      ...
      <div class="form-group row">
          ...
          <option value="C2.2">C2.2</option>
			...
			<input type="submit" name="upload" value="Update" class="btn btn-outline-warning">
